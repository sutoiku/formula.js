...
exports.SUMIF = function(range1, criteria, range2 ) {
		criteria = criteria.replace(/^=[0-9]*$/i, '==' + criteria); // only numeric, prepend == for JS
		criteria = criteria.replace('<>', '!==');
		if(criteria.search(/^[=a-z0-9\s]*[a-z\s][a-z0-9\s]*$/i) >= 0) { // dealing with text
			criteria = " === " + "'" + criteria.replace('=','') + "'"; // in case user specified ="some text"
		}

		if (range2) { // range2 exists means range1 is intended text match
				range2 = utils.parseNumberArray(utils.flatten(range2));
				range1 = range1.map(function(d) { return "'" + d + "'" });
		} else {
			range1 = utils.parseNumberArray(utils.flatten(range1));
		}

	  if (range1 instanceof Error || typeof range2 !== 'undefined' && range2 instanceof Error) {
	    return range1;
	  }
	  var result = 0;
	  for (var i = 0; i < range1.length; i++) {
			if (range2) {
					result += (eval(range1[i] + criteria)) ? range2[i] : 0; // jshint ignore:line
			} else {
				result += (eval(range1[i] + criteria)) ? range1[i] : 0; // jshint ignore:line
			}
	  }
